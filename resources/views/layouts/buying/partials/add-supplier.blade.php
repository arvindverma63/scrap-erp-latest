    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="POST" action="{{ route('admin.supplier.store') }}" class="needs-validation"
                    id="addSupplierForm" novalidate>
                    @csrf

                    <!-- Supplier Type -->
                    <h5 class="mb-3">Supplier Type</h5>
                    <div class="mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="supplier_type" id="individual"
                                value="individual" {{ old('supplier_type', 'individual') == 'individual' ? 'checked' : '' }} required>
                            <label class="form-check-label" for="individual">Individual</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="supplier_type" id="company"
                                value="company" {{ old('supplier_type') == 'company' ? 'checked' : '' }} required>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="country" class="form-select" required>
                                ...
                                {{ old('country', 'Jamaica') == $country->name ? 'selected' : '' }}>
                                {{$country->name}}</option>
                                @endforeach
                            </select>
                        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        form.addEventListener("submit", function (event) {
            ...
            if (form.checkValidity() && isValid) {
                ...
                fetch("{{ route('admin.supplier.storeOnCreateReceipt') }}", {
                    method: "POST",
                    body: formData,
                    headers: {
    ...
    `;
                        ...
                        document.getElementById('supplier_address').value = supplier.street_address ?? '';

                        // ✅ Refresh select2
                        if ($(supplierSelect).hasClass('select2-supplier')) {
